INSERT INTO gold.sig_player_shooting_goals_midfield_sniper (
    match_id,
    match_date,
    home_team_id,
    home_team_name,
...
-- Signal: sig_player_shooting_goals_midfield_sniper
-- Trigger: central-midfielder proxy scores >= 2 non-own goals from outside the box in a finished match.
-- Intent: isolate rare long-range brace outputs from midfield profiles while preserving bilateral
-- match context for interpretation and downstream ranking.
WITH player_role AS (
    SELECT
        match_id,
        person_id,
...
outside_box_shot_stats AS (
    ...
    GROUP BY
        ...
        toInt32(s.player_id),
        toInt32(s.team_id)
    HAVING sum(if(coalesce(s.is_goal, 0) = 1, 1, 0)) >= 2
)
SELECT
    m.match_id,
    m.match_date,
    ...
    toInt32(os.triggered_player_outside_box_goals - 2) AS outside_box_goals_above_threshold,
    toFloat32(coalesce(
        round(
            100.0 * os.triggered_player_outside_box_goals
