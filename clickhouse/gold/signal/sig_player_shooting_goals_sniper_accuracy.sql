INSERT INTO gold.sig_player_shooting_goals_sniper_accuracy (
    match_id,
    match_date,
    home_team_id,
    home_team_name,
    ...
    trigger_threshold_min_shot_accuracy_pct,
    ...
    triggered_player_goals_per_shot,
...
-- Signal: sig_player_shooting_goals_sniper_accuracy
-- Trigger: player records >= 5 total_shots with 100% shot-on-target accuracy in the same match.
-- Intent: isolate high-volume precision shooting performances and preserve bilateral match context
-- for conversion, chance-quality, and tactical interpretation.
...
SELECT
    ...
    toFloat32(100.0) AS trigger_threshold_min_shot_accuracy_pct,
    ...
    toInt32(coalesce(p.total_shots, 0) - coalesce(p.shots_on_target, 0)) AS triggered_player_shots_off_target,
    toFloat32(coalesce(
        round(
    ...
    toFloat32(coalesce(
        ...
        0.0
    )) AS triggered_player_goals_per_shot,
    toFloat32(round(
...
WHERE m.match_finished = 1
  ...
  AND coalesce(p.shots_on_target, 0) = coalesce(p.total_shots, 0)
