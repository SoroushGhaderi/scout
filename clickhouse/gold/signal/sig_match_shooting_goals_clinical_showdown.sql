INSERT INTO gold.sig_match_shooting_goals_clinical_showdown (
    match_id,
    match_date,
    home_team_id,
    home_team_name,
    ...
    opponent_team_id,
    opponent_team_name,
    trigger_threshold_match_total_xg_max,
    trigger_threshold_match_total_goals_min,
    match_total_xg,
    match_total_goals,
    match_goal_minus_xg,
    match_xg_minus_goals,
...
-- Signal: sig_match_shooting_goals_clinical_showdown
-- Intent: detect low chance-quality matches that still produce many goals and emit
--         side-oriented rows for bilateral clinical-finishing diagnostics.
-- Trigger: combined match goals >= 4 and combined match xG < 1.5 in period='All'.
WITH base_stats AS (
    SELECT
    ...
    WHERE m.match_finished = 1
      AND m.match_id > 0
      AND (coalesce(ps.expected_goals_home, 0) + coalesce(ps.expected_goals_away, 0)) < 1.5
...
SELECT
    ...
    toFloat32(1.5) AS trigger_threshold_match_total_xg_max,
    toInt32(4) AS trigger_threshold_match_total_goals_min,
    b.match_total_xg,
    b.match_total_goals,
    toFloat32(round(b.match_total_goals - b.match_total_xg, 3)) AS match_goal_minus_xg,
    toFloat32(round(b.match_total_xg - b.match_total_goals, 3)) AS match_xg_minus_goals,
