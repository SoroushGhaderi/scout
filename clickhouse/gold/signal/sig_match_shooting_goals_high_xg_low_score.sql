INSERT INTO gold.sig_match_shooting_goals_high_xg_low_score (
    match_id,
    match_date,
    home_team_id,
    home_team_name,
...
-- Signal: sig_match_shooting_goals_high_xg_low_score
-- Intent: detect high chance-quality matches that still end with very few goals and emit
--         side-oriented rows for bilateral finishing-underperformance diagnostics.
-- Trigger: combined match xG > 4.0 and combined match goals <= 1 in period='All'.
WITH base_stats AS (
    SELECT
    ...
    WHERE m.match_finished = 1
      AND m.match_id > 0
      AND (coalesce(ps.expected_goals_home, 0) + coalesce(ps.expected_goals_away, 0)) > 4.0
      AND (coalesce(m.home_score, 0) + coalesce(m.away_score, 0)) <= 1
)
...
SELECT
    ...
    toFloat32(4.0) AS trigger_threshold_match_total_xg,
    toInt32(1) AS trigger_threshold_match_total_goals_max,
    b.match_total_xg,
    b.match_total_goals,
