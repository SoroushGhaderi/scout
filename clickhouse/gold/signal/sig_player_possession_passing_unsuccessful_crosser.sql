INSERT INTO gold.sig_player_possession_passing_unsuccessful_crosser (
    match_id,
    match_date,
    home_team_id,
    home_team_name,
...
-- Signal: sig_player_possession_passing_unsuccessful_crosser
-- Trigger: player attempts > 8 crosses with 0% crossing accuracy in a single match.
-- Intent: identify high-volume crossing profiles where delivery quality collapses despite repeated attempts.
...
WHERE m.match_finished = 1
  ...
  AND coalesce(p.cross_attempts, 0) > 8
  AND coalesce(p.accurate_crosses, 0) = 0
