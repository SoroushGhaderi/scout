INSERT INTO gold.sig_player_shooting_goals_the_tap_in_merchant (
    match_id,
    match_date,
    home_team_id,
    home_team_name,
...
-- Signal: sig_player_shooting_goals_the_tap_in_merchant
-- Trigger: player scores >= 2 goals with individual expected_goals > 1.50 in the same match.
-- Intent: isolate multi-goal outputs built on high expected-goals accumulation, indicating
-- repeated close-range or high-probability finishing opportunities.
SELECT
    m.match_id,
    m.match_date,
    m.home_team_id,
