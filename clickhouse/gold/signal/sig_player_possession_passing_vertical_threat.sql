INSERT INTO gold.sig_player_possession_passing_vertical_threat (
    match_id,
    match_date,
    home_team_id,
    home_team_name,
    ...
    opponent_team_id,
    opponent_team_name,
    triggered_player_forward_successful_passes_proxy,
    triggered_player_accurate_passes,
    triggered_player_forward_successful_pass_share_pct,
...
-- Signal: sig_player_possession_passing_vertical_threat
-- Trigger: player directs > 50% of successful passes forward, proxied by final-third passes divided by accurate passes.
-- Intent: identify players whose completed passing profile is unusually vertical and threat-oriented, with bilateral passing and territory context.

WITH
    least(
        greatest(coalesce(p.passes_final_third, 0), 0),
        coalesce(p.accurate_passes, 0)
    ) AS forward_successful_passes_proxy,
    coalesce(
        round(
            100.0 * forward_successful_passes_proxy
            / nullIf(coalesce(p.accurate_passes, 0), 0),
            1
        ),
        0.0
    ) AS forward_successful_pass_share_pct
SELECT
    m.match_id,
    m.match_date,
    m.home_team_id,
    ...
    forward_successful_passes_proxy AS triggered_player_forward_successful_passes_proxy,
    coalesce(p.accurate_passes, 0) AS triggered_player_accurate_passes,
    forward_successful_pass_share_pct AS triggered_player_forward_successful_pass_share_pct,
    ...
    toFloat32(coalesce(p.expected_assists, 0)) AS triggered_player_expected_assists,
...
WHERE m.match_finished = 1
  ...
  AND coalesce(p.accurate_passes, 0) > 0
  AND forward_successful_pass_share_pct > 50

ORDER BY
    triggered_player_forward_successful_pass_share_pct DESC,
    triggered_player_forward_successful_passes_proxy DESC,
