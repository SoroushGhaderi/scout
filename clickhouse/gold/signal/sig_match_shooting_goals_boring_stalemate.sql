INSERT INTO gold.sig_match_shooting_goals_boring_stalemate (
    match_id,
    match_date,
    home_team_id,
    home_team_name,
...
-- Signal: sig_match_shooting_goals_boring_stalemate
-- Intent: detect goalless stalemates with very low combined chance quality and emit
--         side-oriented rows for bilateral shot creation and control diagnostics.
-- Trigger: combined match xG < 0.50 and scoreline 0-0 in period='All'.
WITH base_stats AS (
    SELECT
        ...
        coalesce(ps.shots_on_target_home, 0) + coalesce(ps.shots_on_target_away, 0) AS match_total_shots_on_target,
        coalesce(ps.big_chances_home, 0) + coalesce(ps.big_chances_away, 0) AS match_total_big_chances,
        toFloat32(round(
    ...
    WHERE m.match_finished = 1
      ...
      AND (coalesce(ps.expected_goals_home, 0) + coalesce(ps.expected_goals_away, 0)) < 0.5
)
...
SELECT
    ...
    toFloat32(0.5) AS trigger_threshold_match_total_xg,
    b.match_total_xg,
    b.match_total_goals,
    b.match_total_shots,
