INSERT INTO gold.sig_team_possession_passing_shot_per_possession (
    match_id,
    match_date,
    home_team_id,
    home_team_name,
    ...
    opponent_team_id,
    opponent_team_name,
    trigger_threshold_accurate_passes_per_shot,
    triggered_team_accurate_passes,
    opponent_accurate_passes,
    triggered_team_total_shots,
    opponent_total_shots,
    triggered_team_accurate_passes_per_shot,
    opponent_accurate_passes_per_shot,
    accurate_passes_per_shot_delta,
    triggered_team_shots_per_20_accurate_passes,
    opponent_shots_per_20_accurate_passes,
    shots_per_20_accurate_passes_delta,
...
-- Signal: sig_team_possession_passing_shot_per_possession
-- Trigger: team records at least one shot for every 20 completed passes (accurate_passes / total_shots <= 20).
-- Intent: identify possession phases that are converted into attempts quickly rather than recycled circulation.
...
SELECT
    ...
    toFloat32(20.0) AS trigger_threshold_accurate_passes_per_shot,
    ...
    toFloat32(coalesce(round(coalesce(ps.accurate_passes_home, 0) / greatest(toFloat32(coalesce(ps.total_shots_home, 0)), 1.0), 2), 0.0))
        AS triggered_team_accurate_passes_per_shot,
    toFloat32(coalesce(round(coalesce(ps.accurate_passes_away, 0) / greatest(toFloat32(coalesce(ps.total_shots_away, 0)), 1.0), 2), 0.0))
        AS opponent_accurate_passes_per_shot,
    toFloat32(round(
        coalesce(round(coalesce(ps.accurate_passes_home, 0) / greatest(toFloat32(coalesce(ps.total_shots_home, 0)), 1.0), 2), 0.0)
      - coalesce(round(coalesce(ps.accurate_passes_away, 0) / greatest(toFloat32(coalesce(ps.total_shots_away, 0)), 1.0), 2), 0.0),
        2
    )) AS accurate_passes_per_shot_delta,

    toFloat32(coalesce(round(20.0 * coalesce(ps.total_shots_home, 0) / greatest(toFloat32(coalesce(ps.accurate_passes_home, 0)), 1.0), 3), 0.0))
        AS triggered_team_shots_per_20_accurate_passes,
    toFloat32(coalesce(round(20.0 * coalesce(ps.total_shots_away, 0) / greatest(toFloat32(coalesce(ps.accurate_passes_away, 0)), 1.0), 3), 0.0))
        AS opponent_shots_per_20_accurate_passes,
    toFloat32(round(
        coalesce(round(20.0 * coalesce(ps.total_shots_home, 0) / greatest(toFloat32(coalesce(ps.accurate_passes_home, 0)), 1.0), 3), 0.0)
      - coalesce(round(20.0 * coalesce(ps.total_shots_away, 0) / greatest(toFloat32(coalesce(ps.accurate_passes_away, 0)), 1.0), 3), 0.0),
        3
    )) AS shots_per_20_accurate_passes_delta,
...
WHERE m.match_finished = 1
  ...
  AND coalesce(ps.accurate_passes_home, 0) >= 20
  AND coalesce(ps.accurate_passes_home, 0) <= 20 * coalesce(ps.total_shots_home, 0)
...
WHERE m.match_finished = 1
  ...
  AND coalesce(ps.accurate_passes_away, 0) >= 20
  AND coalesce(ps.accurate_passes_away, 0) <= 20 * coalesce(ps.total_shots_away, 0)

ORDER BY
    assumeNotNull(triggered_team_shots_per_20_accurate_passes) DESC,
    m.match_date DESC,
    m.match_id DESC;
