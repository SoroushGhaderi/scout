INSERT INTO gold.sig_player_possession_passing_perfect_dribbler (
    match_id,
    match_date,
    home_team_id,
    home_team_name,
...
-- Signal: sig_player_possession_passing_perfect_dribbler
-- Trigger: player completes >= 5 successful dribbles with 0 failed dribbles in a single match.
-- Intent: identify players with high-volume flawless dribbling execution, with bilateral possession, passing, and territory context.
...
WHERE m.match_finished = 1
  ...
  AND greatest(coalesce(p.dribble_attempts, 0) - coalesce(p.successful_dribbles, 0), 0) = 0
