INSERT INTO gold.sig_player_shooting_goals_impact_accumulator (
    match_id,
    match_date,
    home_team_id,
    home_team_name,
    ...
    shots_on_target_above_threshold,
...
-- Signal: sig_player_shooting_goals_impact_accumulator
-- Trigger: player records >= 5 shots on target and scores 0 goals in a finished match.
-- Intent: isolate high-accuracy-yet-goalless shooting performances to highlight
-- severe finishing underperformance despite sustained shot pressure on frame.
SELECT
    m.match_id,
    m.match_date,
    m.home_team_id,
    ...
    toInt32(5) AS trigger_threshold_min_shots_on_target,
    ...
    toInt32(coalesce(p.shots_on_target, 0) - 5) AS shots_on_target_above_threshold,
    toFloat32(round(
...
WHERE m.match_finished = 1
  ...
  AND coalesce(p.shots_on_target, 0) >= 5
  AND coalesce(p.goals, 0) = 0

ORDER BY
    triggered_player_shots_on_target DESC,
    shots_on_target_above_threshold DESC,
    expected_goals_wasted DESC,
