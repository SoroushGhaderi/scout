INSERT INTO gold.sig_player_shooting_goals_shot_volume_monster (
    match_id,
    match_date,
    home_team_id,
    home_team_name,
...
-- Signal: sig_player_shooting_goals_shot_volume_monster
-- Trigger: player records >= 8 total shots in the same finished match.
-- Intent: isolate extreme single-match shooting volume at player level with bilateral
-- team and opponent context to separate sustainable pressure from low-quality spam.
...
SELECT
    ...
    toInt32(8) AS trigger_threshold_min_total_shots,
    ...
    toInt32(coalesce(p.total_shots, 0) - 8) AS shots_above_threshold,
...
WHERE m.match_finished = 1
  ...
  AND coalesce(p.total_shots, 0) >= 8
