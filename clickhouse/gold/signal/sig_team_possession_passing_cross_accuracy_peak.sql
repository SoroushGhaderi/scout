INSERT INTO gold.sig_team_possession_passing_cross_accuracy_peak (
    match_id,
    match_date,
    home_team_id,
    home_team_name,
    ...
    trigger_threshold_cross_accuracy_pct,
...
-- Signal: sig_team_possession_passing_cross_accuracy_peak
-- Trigger: Team completes >= 10 crosses with > 40% cross accuracy.
-- Intent: identify teams that combine meaningful crossing volume with elite delivery precision, then compare bilateral territorial and chance-quality context.
...
SELECT
    ...
    10 AS trigger_threshold_cross_attempts,
    toFloat32(40.0) AS trigger_threshold_cross_accuracy_pct,
...
WHERE m.match_finished = 1
  AND m.match_id > 0
  AND coalesce(ps.cross_attempts_home, 0) >= 10
  AND (
...
WHERE m.match_finished = 1
  AND m.match_id > 0
  AND coalesce(ps.cross_attempts_away, 0) >= 10
  AND (
