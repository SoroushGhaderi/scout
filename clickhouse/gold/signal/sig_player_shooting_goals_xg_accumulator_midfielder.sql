...
-- Trigger: player expected_goals > 0.80 while classified as midfielder and not deployed as forward.
-- Intent: isolate high-xG midfield shooting accumulators whose chance load is generated
-- outside explicit forward deployment, with bilateral match context for tactical interpretation.
SELECT
    m.match_id,
    m.match_date,
    m.home_team_id,
...
WHERE m.match_finished = 1
  ...
  AND coalesce(mp.position_id, coalesce(mp.usual_playing_position_id, 0)) != 3
