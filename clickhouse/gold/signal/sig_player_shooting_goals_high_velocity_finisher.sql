INSERT INTO gold.sig_player_shooting_goals_high_velocity_finisher (
    match_id,
    match_date,
    home_team_id,
    home_team_name,
    ...
    trigger_threshold_exclusive_max_minutes_played,
...
-- Signal: sig_player_shooting_goals_high_velocity_finisher
-- Trigger: player scores >= 2 non-own goals while playing fewer than 30 minutes.
-- Intent: isolate high-velocity finishing bursts from low-minute appearances with bilateral
...
SELECT
    ...
    toInt32(30) AS trigger_threshold_exclusive_max_minutes_played,
    ...
    toInt32(30 - coalesce(p.minutes_played, 0)) AS minutes_below_threshold,
    toFloat32(coalesce(round(
        90.0 * coalesce(p.goals, 0)
        / nullIf(toFloat64(coalesce(p.minutes_played, 0)), 0),
        2
    ), 0.0)) AS triggered_player_goals_per_90,
...
WHERE m.match_finished = 1
  ...
  AND coalesce(p.minutes_played, 0) < 30

ORDER BY
    triggered_player_goals_per_90 DESC,
