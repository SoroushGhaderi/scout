INSERT INTO gold.sig_player_possession_passing_one_touch_specialist (
    match_id,
    match_date,
    home_team_id,
    home_team_name,
    ...
    trigger_threshold_max_touches_per_pass,
    triggered_player_total_touches,
    triggered_player_total_passes,
    triggered_player_touches_per_pass,
...
-- Signal: sig_player_possession_passing_one_touch_specialist
-- Trigger: player has >= 50 total passes with < 1.2 touches per pass.
-- Intent: identify rapid one-touch circulation specialists who sustain high passing volume with minimal ball dwell time, in bilateral possession and passing context.

WITH
    coalesce(
        round(
            coalesce(p.touches, 0)
            / nullIf(coalesce(p.total_passes, 0), 0),
            2
        ),
        0.0
    ) AS touches_per_pass_ratio,
    coalesce(
        p.pass_accuracy,
        round(
...
SELECT
    ...
    50 AS trigger_threshold_total_passes,
    toFloat32(1.2) AS trigger_threshold_max_touches_per_pass,

    coalesce(p.touches, 0) AS triggered_player_total_touches,
    coalesce(p.total_passes, 0) AS triggered_player_total_passes,
    touches_per_pass_ratio AS triggered_player_touches_per_pass,
...
WHERE m.match_finished = 1
  ...
  AND touches_per_pass_ratio < 1.2

ORDER BY
    triggered_player_touches_per_pass ASC,
