INSERT INTO gold.sig_match_shooting_goals_high_volume_low_target (
    match_id,
    match_date,
    home_team_id,
    home_team_name,
    ...
    opponent_team_id,
    opponent_team_name,
    trigger_threshold_match_total_shots_min,
    trigger_threshold_match_total_shots_on_target_max,
    match_total_shots,
...
-- Signal: sig_match_shooting_goals_high_volume_low_target
-- Intent: detect finished matches with very high combined shot volume but very low combined
--         shots-on-target output, then emit side-oriented bilateral chance and execution context.
-- Trigger: combined total shots >= 40 and combined shots on target < 5 in period='All'.
WITH base_stats AS (
    SELECT
    ...
    WHERE m.match_finished = 1
      AND m.match_id > 0
      AND (coalesce(ps.total_shots_home, 0) + coalesce(ps.total_shots_away, 0)) >= 40
      AND (coalesce(ps.shots_on_target_home, 0) + coalesce(ps.shots_on_target_away, 0)) < 5
)
...
SELECT
    ...
    toInt32(40) AS trigger_threshold_match_total_shots_min,
    toInt32(4) AS trigger_threshold_match_total_shots_on_target_max,
    b.match_total_shots,
...
ORDER BY
    ...
    match_total_shots_on_target ASC,
    match_date DESC,
    match_id DESC,
    triggered_side;
