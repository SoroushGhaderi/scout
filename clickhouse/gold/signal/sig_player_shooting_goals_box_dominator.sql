INSERT INTO gold.sig_player_shooting_goals_box_dominator (
    match_id,
    match_date,
    home_team_id,
    home_team_name,
...
-- Signal: sig_player_shooting_goals_box_dominator
-- Trigger: player records >= 15 touches in opposition box and >= 5 total shots.
-- Intent: isolate sustained penalty-area dominance by players who repeatedly receive
-- touches in high-value zones and still convert that presence into direct shot volume.
...
SELECT
    ...
    toInt32(15) AS trigger_threshold_min_touches_opposition_box,
    ...
    toInt32(coalesce(p.touches_opp_box, 0) - 15) AS box_touches_above_threshold,
    toInt32(coalesce(p.total_shots, 0) - 5) AS shots_above_threshold,
    toFloat32(coalesce(
        round(
            coalesce(p.touches_opp_box, 0)
            / nullIf(toFloat64(coalesce(p.total_shots, 0)), 0),
            2
        ),
        0.0
    )) AS box_touches_per_shot,
