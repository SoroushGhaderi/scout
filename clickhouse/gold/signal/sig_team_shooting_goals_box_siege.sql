INSERT INTO gold.sig_team_shooting_goals_box_siege (
    match_id,
    match_date,
    home_team_id,
    home_team_name,
    ...
    opponent_team_id,
    opponent_team_name,
    trigger_threshold_min_shots_inside_box,
    triggered_team_shots_inside_box,
    opponent_shots_inside_box,
    shots_inside_box_delta,
    triggered_team_total_shots,
    opponent_total_shots,
    triggered_team_inside_box_shot_share_pct,
    opponent_inside_box_shot_share_pct,
    inside_box_shot_share_delta_pct,
    triggered_team_inside_box_shots_on_target,
    opponent_inside_box_shots_on_target,
    triggered_team_inside_box_shot_accuracy_pct,
    opponent_inside_box_shot_accuracy_pct,
    inside_box_shot_accuracy_delta_pct,
    triggered_team_inside_box_goals,
    opponent_inside_box_goals,
    triggered_team_inside_box_xg,
    opponent_inside_box_xg,
    triggered_team_xg,
    opponent_xg,
    xg_delta,
...
-- Signal: sig_team_shooting_goals_box_siege
-- Trigger: team records >= 20 inside-box shots in a finished match at period='All'.
-- Intent: capture inside-box shot-volume siege and preserve bilateral execution, chance-quality, and control context.
...
SELECT
    ...
    toInt32(20) AS trigger_threshold_min_shots_inside_box,
    toInt32(coalesce(home_ib.team_shots_inside_box, 0)) AS triggered_team_shots_inside_box,
    toInt32(coalesce(away_ib.team_shots_inside_box, 0)) AS opponent_shots_inside_box,
    toInt32(
        coalesce(home_ib.team_shots_inside_box, 0) - coalesce(away_ib.team_shots_inside_box, 0)
    ) AS shots_inside_box_delta,
    ...
    toFloat32(coalesce(round(
        100.0 * coalesce(home_ib.team_shots_inside_box, 0)
        / nullIf(toFloat64(coalesce(ps.total_shots_home, 0)), 0),
        1
    ), 0.0)) AS triggered_team_inside_box_shot_share_pct,
    toFloat32(coalesce(round(
        100.0 * coalesce(away_ib.team_shots_inside_box, 0)
        / nullIf(toFloat64(coalesce(ps.total_shots_away, 0)), 0),
        1
    ), 0.0)) AS opponent_inside_box_shot_share_pct,
    toFloat32(round(
        coalesce(round(
            100.0 * coalesce(home_ib.team_shots_inside_box, 0)
            / nullIf(toFloat64(coalesce(ps.total_shots_home, 0)), 0),
      ...
      - coalesce(round(
        ...
        1
    )) AS inside_box_shot_share_delta_pct,

    toInt32(coalesce(home_ib.team_inside_box_shots_on_target, 0)) AS triggered_team_inside_box_shots_on_target,
    toInt32(coalesce(away_ib.team_inside_box_shots_on_target, 0)) AS opponent_inside_box_shots_on_target,
    toFloat32(coalesce(round(
        100.0 * coalesce(home_ib.team_inside_box_shots_on_target, 0)
        / nullIf(toFloat64(coalesce(home_ib.team_shots_inside_box, 0)), 0),
        1
    ), 0.0)) AS triggered_team_inside_box_shot_accuracy_pct,
    toFloat32(coalesce(round(
        100.0 * coalesce(away_ib.team_inside_box_shots_on_target, 0)
        / nullIf(toFloat64(coalesce(away_ib.team_shots_inside_box, 0)), 0),
        1
    ), 0.0)) AS opponent_inside_box_shot_accuracy_pct,
    toFloat32(round(
        coalesce(round(
            100.0 * coalesce(home_ib.team_inside_box_shots_on_target, 0)
            / nullIf(toFloat64(coalesce(home_ib.team_shots_inside_box, 0)), 0),
      ...
      - coalesce(round(
        ...
        1
    )) AS inside_box_shot_accuracy_delta_pct,

    toInt32(coalesce(home_ib.team_inside_box_goals, 0)) AS triggered_team_inside_box_goals,
    toInt32(coalesce(away_ib.team_inside_box_goals, 0)) AS opponent_inside_box_goals,
    toFloat32(coalesce(home_ib.team_inside_box_xg, 0.0)) AS triggered_team_inside_box_xg,
    toFloat32(coalesce(away_ib.team_inside_box_xg, 0.0)) AS opponent_inside_box_xg,
...
WHERE m.match_finished = 1
  AND m.match_id > 0
  AND coalesce(home_ib.team_shots_inside_box, 0) >= 20
...
SELECT
    ...
    toInt32(20) AS trigger_threshold_min_shots_inside_box,
    toInt32(coalesce(away_ib.team_shots_inside_box, 0)) AS triggered_team_shots_inside_box,
    toInt32(coalesce(home_ib.team_shots_inside_box, 0)) AS opponent_shots_inside_box,
    toInt32(
        coalesce(away_ib.team_shots_inside_box, 0) - coalesce(home_ib.team_shots_inside_box, 0)
    ) AS shots_inside_box_delta,
    ...
    toInt32(coalesce(away_ib.team_inside_box_shots_on_target, 0)) AS triggered_team_inside_box_shots_on_target,
    toInt32(coalesce(home_ib.team_inside_box_shots_on_target, 0)) AS opponent_inside_box_shots_on_target,
    toFloat32(coalesce(round(
        100.0 * coalesce(away_ib.team_inside_box_shots_on_target, 0)
        / nullIf(toFloat64(coalesce(away_ib.team_shots_inside_box, 0)), 0),
        1
    ...
    toInt32(coalesce(away_ib.team_inside_box_goals, 0)) AS triggered_team_inside_box_goals,
    toInt32(coalesce(home_ib.team_inside_box_goals, 0)) AS opponent_inside_box_goals,
    toFloat32(coalesce(away_ib.team_inside_box_xg, 0.0)) AS triggered_team_inside_box_xg,
    toFloat32(coalesce(home_ib.team_inside_box_xg, 0.0)) AS opponent_inside_box_xg,
...
WHERE m.match_finished = 1
  AND m.match_id > 0
  AND coalesce(away_ib.team_shots_inside_box, 0) >= 20

ORDER BY
    triggered_team_shots_inside_box DESC,
    inside_box_shot_share_delta_pct DESC,
    m.match_date DESC,
    m.match_id DESC;
