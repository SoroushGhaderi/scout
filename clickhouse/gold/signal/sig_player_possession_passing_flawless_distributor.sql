INSERT INTO gold.sig_player_possession_passing_flawless_distributor (
    match_id,
    match_date,
    home_team_id,
    home_team_name,
    ...
    opponent_team_id,
    opponent_team_name,
    trigger_threshold_pass_attempts,
...
-- ============================================================
-- Signal: sig_player_possession_passing_flawless_distributor
-- Intent: Detect players who deliver flawless high-volume distribution
--         and expose team/opponent possession-passing context.
-- Trigger: player total_passes >= 40 AND accurate_passes = total_passes.
-- ============================================================
...
SELECT
    ...
    40 AS trigger_threshold_pass_attempts,
    toFloat32(100.0) AS trigger_threshold_pass_accuracy_pct,
...
INNER JOIN silver.match AS m FINAL
    ON m.match_id = p.match_id
LEFT JOIN silver.period_stat AS ps FINAL
    ON ps.match_id = p.match_id
   AND ps.period = 'All'
WHERE m.match_finished = 1
  AND m.match_id > 0
  AND (p.team_id = m.home_team_id OR p.team_id = m.away_team_id)
  AND coalesce(p.total_passes, 0) >= 40
