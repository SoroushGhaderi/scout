INSERT INTO gold.sig_team_shooting_goals_offensive_masterclass (
    match_id,
    match_date,
    home_team_id,
    home_team_name,
...
-- Signal: sig_team_shooting_goals_offensive_masterclass
-- Trigger: Team records xG per shot > 0.20 in a finished match (`period = 'All'`).
...
SELECT
    ...
    toFloat32(0.20) AS trigger_threshold_min_xg_per_shot,
...
WHERE m.match_finished = 1
  ...
  AND coalesce(ps.expected_goals_home, 0.0)
        / nullIf(toFloat64(coalesce(ps.total_shots_home, 0)), 0) > 0.20
...
WHERE m.match_finished = 1
  ...
  AND coalesce(ps.expected_goals_away, 0.0)
        / nullIf(toFloat64(coalesce(ps.total_shots_away, 0)), 0) > 0.20
