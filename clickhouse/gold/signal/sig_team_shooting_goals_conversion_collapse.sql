INSERT INTO gold.sig_team_shooting_goals_conversion_collapse (
    match_id,
    match_date,
    home_team_id,
    home_team_name,
    ...
    triggered_team_unconverted_shots_on_target,
    opponent_unconverted_shots_on_target,
    unconverted_shots_on_target_delta,
    triggered_team_xg,
    opponent_xg,
    xg_delta,
...
-- Signal: sig_team_shooting_goals_conversion_collapse
-- Trigger: Team records >= 10 shots on target but scores exactly 1 goal in a finished match (`period = 'All'`).
-- Intent: Detect severe team-level finishing collapse despite repeated on-target execution.
...
SELECT
    ...
    toInt32(1) AS trigger_required_goals,
    ...
    toInt32(greatest(coalesce(ps.shots_on_target_home, 0) - coalesce(m.home_score, 0), 0))
        AS triggered_team_unconverted_shots_on_target,
    toInt32(greatest(coalesce(ps.shots_on_target_away, 0) - coalesce(m.away_score, 0), 0))
        AS opponent_unconverted_shots_on_target,
    toInt32(
        greatest(coalesce(ps.shots_on_target_home, 0) - coalesce(m.home_score, 0), 0)
      - greatest(coalesce(ps.shots_on_target_away, 0) - coalesce(m.away_score, 0), 0)
    ) AS unconverted_shots_on_target_delta,
...
ORDER BY
    triggered_team_shots_on_target DESC,
    triggered_team_goal_conversion_pct ASC,
    m.match_date DESC,
    m.match_id DESC;
