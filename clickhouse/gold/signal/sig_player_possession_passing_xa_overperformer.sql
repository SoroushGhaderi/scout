INSERT INTO gold.sig_player_possession_passing_xa_overperformer (
    match_id,
    match_date,
    home_team_id,
    home_team_name,
...
-- Signal: sig_player_possession_passing_xa_overperformer
-- Trigger: player records 2+ assists with expected assists (xA) < 0.5.
-- Intent: identify players whose assist output materially outpaced underlying expected-assist volume, preserving bilateral passing and territorial context.
...
WHERE m.match_finished = 1
  ...
  AND coalesce(p.expected_assists, 0.0) < 0.5

ORDER BY
    triggered_player_assists DESC,
    triggered_player_expected_assists ASC,
