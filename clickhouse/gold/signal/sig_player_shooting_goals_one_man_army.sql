INSERT INTO gold.sig_player_shooting_goals_one_man_army (
    match_id,
    match_date,
    home_team_id,
    home_team_name,
    ...
    opponent_team_id,
    opponent_team_name,
    trigger_threshold_team_goals,
    trigger_threshold_opponent_goals,
    trigger_threshold_player_goals,
    triggered_player_only_team_goals,
    triggered_player_first_goal_minute,
    triggered_player_first_goal_added_time,
    triggered_player_first_goal_effective_minute,
    triggered_team_score_before_only_goal,
    opponent_score_before_only_goal,
    triggered_team_score_after_only_goal,
    opponent_score_after_only_goal,
    triggered_player_goal_contribution_pct,
    triggered_player_goals_above_threshold,
...
-- Signal: sig_player_shooting_goals_one_man_army
-- Trigger: player scores the only non-own goal for their team in a finished 1-0 win.
-- Intent: isolate one-player match-winning finishing events and preserve player plus bilateral
-- team/opponent shooting context.
WITH one_man_army_goal_events AS (
    SELECT
        s.match_id,
...
player_one_man_army_goals AS (
    SELECT
        e.match_id,
        e.team_id,
        e.player_id,
        argMin(e.player_name, tuple(e.goal_effective_minute, e.shot_id)) AS triggered_player_name,
        toInt32(count()) AS triggered_player_only_team_goals,
        toInt32(argMin(
            e.goal_minute,
            tuple(e.goal_effective_minute, e.shot_id)
        )) AS triggered_player_first_goal_minute,
        toInt32(argMin(
            e.goal_added_time,
            tuple(e.goal_effective_minute, e.shot_id)
        )) AS triggered_player_first_goal_added_time,
        toInt32(min(e.goal_effective_minute)) AS triggered_player_first_goal_effective_minute,
        toInt32(argMin(
            e.triggered_team_score_before,
            tuple(e.goal_effective_minute, e.shot_id)
        )) AS triggered_team_score_before_only_goal,
        toInt32(argMin(
            e.opponent_score_before,
            tuple(e.goal_effective_minute, e.shot_id)
        )) AS opponent_score_before_only_goal,
        toInt32(argMin(
            e.triggered_team_score_after,
            tuple(e.goal_effective_minute, e.shot_id)
        )) AS triggered_team_score_after_only_goal,
        toInt32(argMin(
            e.opponent_score_after,
            tuple(e.goal_effective_minute, e.shot_id)
        )) AS opponent_score_after_only_goal
    FROM one_man_army_goal_events AS e
    GROUP BY
        e.match_id,
        e.team_id,
        e.player_id
...
SELECT
    ...
    coalesce(oma.triggered_player_name, p.player_name, 'Unknown') AS triggered_player_name,
    ...
    toInt32(1) AS trigger_threshold_team_goals,
    toInt32(0) AS trigger_threshold_opponent_goals,
    toInt32(1) AS trigger_threshold_player_goals,
    oma.triggered_player_only_team_goals,
    oma.triggered_player_first_goal_minute,
    oma.triggered_player_first_goal_added_time,
    oma.triggered_player_first_goal_effective_minute,
    oma.triggered_team_score_before_only_goal,
    oma.opponent_score_before_only_goal,
    oma.triggered_team_score_after_only_goal,
    oma.opponent_score_after_only_goal,
    toFloat32(coalesce(round(
        100.0 * oma.triggered_player_only_team_goals
        / nullIf(toFloat64(if(
        ...
        1
    ), 0.0)) AS triggered_player_goal_contribution_pct,
    toInt32(coalesce(p.goals, 0) - 1) AS triggered_player_goals_above_threshold,
...
FROM player_one_man_army_goals AS oma
INNER JOIN silver.player_match_stat AS p
    ON p.match_id = oma.match_id
   AND p.player_id = oma.player_id
   AND p.team_id = oma.team_id
INNER JOIN silver.match AS m
    ON m.match_id = oma.match_id
LEFT JOIN silver.period_stat AS ps
    ON ps.match_id = oma.match_id
   AND ps.period = 'All'
WHERE m.match_finished = 1
  AND m.match_id > 0
  AND p.player_id > 0
  AND (p.team_id = m.home_team_id OR p.team_id = m.away_team_id)
  AND oma.triggered_player_only_team_goals = 1
  AND coalesce(p.goals, 0) = 1
  AND if(
        p.team_id = m.home_team_id,
        coalesce(m.home_score, 0),
        coalesce(m.away_score, 0)
...
ORDER BY
    oma.triggered_player_first_goal_effective_minute ASC,
    triggered_player_goal_minus_expected_goals DESC,
    coalesce(p.expected_goals, 0.0) DESC,
    m.match_date DESC,
    m.match_id DESC;
