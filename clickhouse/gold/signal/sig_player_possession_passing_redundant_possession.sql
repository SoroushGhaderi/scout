INSERT INTO gold.sig_player_possession_passing_redundant_possession (
    match_id,
    match_date,
    home_team_id,
    home_team_name,
...
-- Signal: sig_player_possession_passing_redundant_possession
-- Trigger: player records >= 80 total passes with 0 passes into the final third.
-- Intent: identify high-volume circulation profiles with no final-third progression contribution.
...
SELECT
    ...
    multiIf(
        ...
        coalesce(mp.usual_playing_position_id, 0) = 3, 'forward',
        'other'
...
WHERE m.match_finished = 1
  ...
  AND coalesce(p.total_passes, 0) >= 80
  AND coalesce(p.passes_final_third, 0) = 0
