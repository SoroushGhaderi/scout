INSERT INTO gold.sig_team_shooting_goals_clinical_finishing_streak (
    match_id,
    match_date,
    home_team_id,
    home_team_name,
    ...
    trigger_threshold_goal_conversion_pct,
...
-- Signal: sig_team_shooting_goals_clinical_finishing_streak
-- Trigger: team has shots_on_target >= 3 and goals = shots_on_target in a finished match at period='All'.
-- Intent: identify perfect on-target conversion streaks at team level with bilateral shot-quality context.
...
SELECT
    ...
    toInt32(3) AS trigger_threshold_min_shots_on_target,
    toFloat32(100.0) AS trigger_threshold_goal_conversion_pct,
...
WHERE m.match_finished = 1
  AND m.match_id > 0
  AND coalesce(ps.shots_on_target_home, 0) >= 3
  AND coalesce(m.home_score, 0) = coalesce(ps.shots_on_target_home, 0)
...
WHERE m.match_finished = 1
  AND m.match_id > 0
  AND coalesce(ps.shots_on_target_away, 0) >= 3
  AND coalesce(m.away_score, 0) = coalesce(ps.shots_on_target_away, 0)
