INSERT INTO gold.sig_match_shooting_goals_unlucky_game (
    match_id,
    match_date,
    home_team_id,
    home_team_name,
...
-- Signal: sig_match_shooting_goals_unlucky_game
-- Intent: detect matches with repeated woodwork denials and emit side-oriented rows for
--         bilateral finishing, chance-quality, and territorial-control diagnostics.
-- Trigger: combined match woodwork hits >= 4 in period='All'.
WITH base_stats AS (
    SELECT
    ...
    WHERE m.match_finished = 1
      AND m.match_id > 0
      AND (coalesce(ps.shots_woodwork_home, 0) + coalesce(ps.shots_woodwork_away, 0)) >= 4
)
...
SELECT
    ...
    toInt32(4) AS trigger_threshold_match_total_shots_woodwork,
