INSERT INTO gold.sig_team_shooting_goals_shooting_gallery (
    match_id,
    match_date,
    home_team_id,
    home_team_name,
...
-- Signal: sig_team_shooting_goals_shooting_gallery
-- Trigger: Team records >= 25 total shots in a single match (`period = 'All'`).
-- Intent: Identify extreme shot-volume matches and contextualize whether volume translated to shot quality,
--         territorial pressure, and attacking efficiency versus the opponent baseline.
...
SELECT
    ...
    toInt32(25) AS trigger_threshold_total_shots,
    ...
    toFloat32(coalesce(round(100.0 * coalesce(ps.shots_on_target_home, 0)
        / nullIf(coalesce(ps.total_shots_home, 0), 0), 1), 0.0)) AS triggered_team_on_target_ratio_pct,
    toFloat32(coalesce(round(100.0 * coalesce(ps.shots_on_target_away, 0)
        / nullIf(coalesce(ps.total_shots_away, 0), 0), 1), 0.0)) AS opponent_on_target_ratio_pct,
    toFloat32(round(
        coalesce(round(100.0 * coalesce(ps.shots_on_target_home, 0)
            / nullIf(coalesce(ps.total_shots_home, 0), 0), 1), 0.0)
      - coalesce(round(100.0 * coalesce(ps.shots_on_target_away, 0)
            / nullIf(coalesce(ps.total_shots_away, 0), 0), 1), 0.0),
        1
    ...
    toFloat32(coalesce(round(coalesce(ps.expected_goals_home, 0)
        / nullIf(toFloat32(coalesce(ps.total_shots_home, 0)), 0.0), 3), 0.0)) AS triggered_team_xg_per_shot,
    toFloat32(coalesce(round(coalesce(ps.expected_goals_away, 0)
        / nullIf(toFloat32(coalesce(ps.total_shots_away, 0)), 0.0), 3), 0.0)) AS opponent_xg_per_shot,
    ...
    toFloat32(coalesce(round(100.0 * coalesce(ps.accurate_passes_home, 0)
        / nullIf(coalesce(ps.pass_attempts_home, 0), 0), 1), 0.0)) AS triggered_team_pass_accuracy_pct,
    toFloat32(coalesce(round(100.0 * coalesce(ps.accurate_passes_away, 0)
        / nullIf(coalesce(ps.pass_attempts_away, 0), 0), 1), 0.0)) AS opponent_pass_accuracy_pct,
    toFloat32(round(
        coalesce(round(100.0 * coalesce(ps.accurate_passes_home, 0)
            / nullIf(coalesce(ps.pass_attempts_home, 0), 0), 1), 0.0)
      - coalesce(round(100.0 * coalesce(ps.accurate_passes_away, 0)
            / nullIf(coalesce(ps.pass_attempts_away, 0), 0), 1), 0.0),
        1
...
WHERE m.match_finished = 1
  AND m.match_id > 0
  AND coalesce(ps.total_shots_home, 0) >= 25
...
SELECT
    ...
    toFloat32(coalesce(round(100.0 * coalesce(ps.shots_on_target_away, 0)
        / nullIf(coalesce(ps.total_shots_away, 0), 0), 1), 0.0)) AS triggered_team_on_target_ratio_pct,
    toFloat32(coalesce(round(100.0 * coalesce(ps.shots_on_target_home, 0)
        / nullIf(coalesce(ps.total_shots_home, 0), 0), 1), 0.0)) AS opponent_on_target_ratio_pct,
    toFloat32(round(
        coalesce(round(100.0 * coalesce(ps.shots_on_target_away, 0)
            / nullIf(coalesce(ps.total_shots_away, 0), 0), 1), 0.0)
      - coalesce(round(100.0 * coalesce(ps.shots_on_target_home, 0)
    ...
    toFloat32(coalesce(round(coalesce(ps.expected_goals_away, 0)
        / nullIf(toFloat32(coalesce(ps.total_shots_away, 0)), 0.0), 3), 0.0)) AS triggered_team_xg_per_shot,
    toFloat32(coalesce(round(coalesce(ps.expected_goals_home, 0)
        / nullIf(toFloat32(coalesce(ps.total_shots_home, 0)), 0.0), 3), 0.0)) AS opponent_xg_per_shot,
    ...
    toFloat32(coalesce(round(100.0 * coalesce(ps.accurate_passes_away, 0)
        / nullIf(coalesce(ps.pass_attempts_away, 0), 0), 1), 0.0)) AS triggered_team_pass_accuracy_pct,
    toFloat32(coalesce(round(100.0 * coalesce(ps.accurate_passes_home, 0)
        / nullIf(coalesce(ps.pass_attempts_home, 0), 0), 1), 0.0)) AS opponent_pass_accuracy_pct,
    toFloat32(round(
        coalesce(round(100.0 * coalesce(ps.accurate_passes_away, 0)
            / nullIf(coalesce(ps.pass_attempts_away, 0), 0), 1), 0.0)
      - coalesce(round(100.0 * coalesce(ps.accurate_passes_home, 0)
...
WHERE m.match_finished = 1
  AND m.match_id > 0
  AND coalesce(ps.total_shots_away, 0) >= 25
