INSERT INTO gold.sig_match_shooting_goals_shot_efficiency_parity (
    match_id,
    match_date,
    home_team_id,
    home_team_name,
    ...
    opponent_team_id,
    opponent_team_name,
    trigger_threshold_abs_shots_on_target_delta,
    trigger_threshold_abs_goal_delta,
    match_total_goals,
    ...
    triggered_team_goal_per_shot_on_target_pct,
    opponent_goal_per_shot_on_target_pct,
    goal_per_shot_on_target_delta_pct,
...
-- Signal: sig_match_shooting_goals_shot_efficiency_parity
-- Intent: detect finished matches where both teams post identical shots-on-target and goal counts,
--         then emit side-oriented parity diagnostics for finishing efficiency and chance quality.
-- Trigger: shots_on_target_home = shots_on_target_away and home_score = away_score at period='All'.
WITH base_stats AS (
    SELECT
    ...
    WHERE m.match_finished = 1
      AND m.match_id > 0
      AND coalesce(ps.shots_on_target_home, 0) = coalesce(ps.shots_on_target_away, 0)
...
SELECT
    ...
    toInt32(0) AS trigger_threshold_abs_shots_on_target_delta,
    toInt32(0) AS trigger_threshold_abs_goal_delta,
    b.match_total_goals,
    b.match_total_shots_on_target,
    b.match_total_shots,
    b.match_total_xg,
    b.shots_on_target_home AS home_shots_on_target,
    b.shots_on_target_away AS away_shots_on_target,
    b.home_goals,
    b.away_goals,
    ...
    toFloat32(coalesce(round(
        100.0 * b.home_goals / nullIf(toFloat64(b.shots_on_target_home), 0),
        1
    ), 0.0)) AS triggered_team_goal_per_shot_on_target_pct,
    toFloat32(coalesce(round(
        100.0 * b.away_goals / nullIf(toFloat64(b.shots_on_target_away), 0),
        1
    ), 0.0)) AS opponent_goal_per_shot_on_target_pct,
    toFloat32(round(
        coalesce(round(100.0 * b.home_goals / nullIf(toFloat64(b.shots_on_target_home), 0), 1), 0.0)
        - coalesce(round(100.0 * b.away_goals / nullIf(toFloat64(b.shots_on_target_away), 0), 1), 0.0),
        1
    )) AS goal_per_shot_on_target_delta_pct,
