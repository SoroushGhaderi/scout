INSERT INTO gold.sig_player_shooting_goals_shot_conversion_peak (
    match_id,
    match_date,
    home_team_id,
    home_team_name,
    ...
    opponent_team_id,
    opponent_team_name,
    trigger_threshold_exact_goals,
    trigger_threshold_exact_total_shots,
    triggered_player_goals,
    triggered_player_total_shots,
    triggered_player_shots_on_target,
    triggered_player_non_goal_shots,
...
-- Signal: sig_player_shooting_goals_shot_conversion_peak
-- Trigger: player scores exactly 2 goals from exactly 2 total shots in the same finished match.
-- Intent: isolate perfect two-shot/two-goal finishing peaks with bilateral team-opponent context.
...
SELECT
    ...
    toInt32(2) AS trigger_threshold_exact_goals,
    toInt32(2) AS trigger_threshold_exact_total_shots,
    ...
    toInt32(coalesce(p.total_shots, 0) - coalesce(p.goals, 0)) AS triggered_player_non_goal_shots,
...
WHERE m.match_finished = 1
  ...
  AND coalesce(p.total_shots, 0) = 2

ORDER BY
    triggered_player_expected_goals_per_shot DESC,
