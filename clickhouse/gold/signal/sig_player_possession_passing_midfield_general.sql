INSERT INTO gold.sig_player_possession_passing_midfield_general (
    match_id,
    match_date,
    home_team_id,
    home_team_name,
    ...
    opponent_team_id,
    opponent_team_name,
    triggered_player_middle_third_touches_proxy,
...
-- Signal: sig_player_possession_passing_midfield_general
-- Trigger: player records > 100 total touches in the middle third.
-- Intent: identify high-involvement midfield controllers using a middle-third touch proxy, with bilateral passing and territorial context.
...
SELECT
    ...
    greatest(coalesce(p.touches, 0) - coalesce(p.touches_opp_box, 0), 0) AS triggered_player_middle_third_touches_proxy,
...
WHERE m.match_finished = 1
  ...
  AND greatest(coalesce(p.touches, 0) - coalesce(p.touches_opp_box, 0), 0) > 100
