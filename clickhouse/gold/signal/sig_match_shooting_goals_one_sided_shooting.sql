INSERT INTO gold.sig_match_shooting_goals_one_sided_shooting (
    match_id,
    match_date,
    home_team_id,
    home_team_name,
    ...
    opponent_team_id,
    opponent_team_name,
    trigger_threshold_min_triggered_team_shots_on_target,
    trigger_threshold_max_opponent_shots_on_target,
    match_total_shots_on_target,
    triggered_team_shots_on_target,
    opponent_shots_on_target,
    shot_on_target_delta,
    triggered_team_shots_on_target_share_pct,
    opponent_shots_on_target_share_pct,
...
-- Signal: sig_match_shooting_goals_one_sided_shooting
-- Intent: Detect finished matches where one side records every on-target shot attempt
--         (`> 0` vs `0`) and expose bilateral shooting, chance-quality, and control context.
-- Trigger: Triggered team shots on target > 0 and opponent shots on target = 0 (`period = 'All'`).
WITH base_stats AS (
    SELECT
...
SELECT
    ...
    toInt32(1) AS trigger_threshold_min_triggered_team_shots_on_target,
    toInt32(0) AS trigger_threshold_max_opponent_shots_on_target,
    b.shots_on_target_home + b.shots_on_target_away AS match_total_shots_on_target,
    ...
    toFloat32(coalesce(round(
        100.0 * b.shots_on_target_home / nullIf(toFloat64(b.shots_on_target_home + b.shots_on_target_away), 0),
        1
    ), 0.0)) AS triggered_team_shots_on_target_share_pct,
    toFloat32(coalesce(round(
        100.0 * b.shots_on_target_away / nullIf(toFloat64(b.shots_on_target_home + b.shots_on_target_away), 0),
        1
    ), 0.0)) AS opponent_shots_on_target_share_pct,
...
WHERE b.shots_on_target_home > 0
  AND b.shots_on_target_away = 0
...
FROM base_stats AS b
WHERE b.shots_on_target_away > 0
  AND b.shots_on_target_home = 0;
