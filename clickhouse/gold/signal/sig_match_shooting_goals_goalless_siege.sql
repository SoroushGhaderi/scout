INSERT INTO gold.sig_match_shooting_goals_goalless_siege (
    match_id,
    match_date,
    home_team_id,
    home_team_name,
    ...
    opponent_team_id,
    opponent_team_name,
    trigger_threshold_min_team_shots,
    match_total_goals,
    match_total_shots,
    match_total_xg,
    triggered_team_meets_shot_threshold,
    opponent_meets_shot_threshold,
    both_teams_meet_shot_threshold,
...
-- Signal: sig_match_shooting_goals_goalless_siege
-- Intent: detect goalless matches where at least one side generates extreme shot volume and emit
--         side-oriented rows for bilateral finishing, chance-quality, and control diagnostics.
-- Trigger: full-time scoreline is 0-0 and at least one team records >= 25 total shots in period='All'.
WITH base_stats AS (
    SELECT
    ...
    WHERE m.match_finished = 1
      ...
      AND (
          ...
          OR coalesce(ps.total_shots_away, 0) >= 25
      )
)
...
SELECT
    ...
    toInt32(25) AS trigger_threshold_min_team_shots,
    b.match_total_goals,
    b.match_total_shots,
    b.match_total_xg,
    toInt8(if(b.total_shots_home >= 25, 1, 0)) AS triggered_team_meets_shot_threshold,
    toInt8(if(b.total_shots_away >= 25, 1, 0)) AS opponent_meets_shot_threshold,
    toInt8(if(b.total_shots_home >= 25 AND b.total_shots_away >= 25, 1, 0)) AS both_teams_meet_shot_threshold,
...
SELECT
    ...
    b.match_total_shots,
    b.match_total_xg,
    toInt8(if(b.total_shots_away >= 25, 1, 0)) AS triggered_team_meets_shot_threshold,
    toInt8(if(b.total_shots_home >= 25, 1, 0)) AS opponent_meets_shot_threshold,
    toInt8(if(b.total_shots_home >= 25 AND b.total_shots_away >= 25, 1, 0)) AS both_teams_meet_shot_threshold,
