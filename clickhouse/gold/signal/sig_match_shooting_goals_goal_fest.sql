INSERT INTO gold.sig_match_shooting_goals_goal_fest (
    match_id,
    match_date,
    home_team_id,
    home_team_name,
    ...
    opponent_goals,
    goal_gap,
    triggered_team_goal_share_pct,
    opponent_goal_share_pct,
    goal_share_delta_pct,
    triggered_team_xg,
    opponent_xg,
    xg_gap,
...
-- Signal: sig_match_shooting_goals_goal_fest
-- Intent: detect high-scoring goal-fest matches and emit side-oriented rows with bilateral
--         finishing, chance-quality, and control context for downstream team analysis.
...
WITH base_stats AS (
    ...
    WHERE m.match_finished = 1
      AND m.match_id > 0
      AND (coalesce(m.home_score, 0) + coalesce(m.away_score, 0)) >= 6
)
...
SELECT
    ...
    toInt32(6) AS trigger_threshold_match_total_goals,
    b.match_total_goals,
    b.match_total_xg,
    ...
    toFloat32(coalesce(round(
        100.0 * b.home_goals / nullIf(toFloat64(b.match_total_goals), 0),
        1
    ), 0.0)) AS triggered_team_goal_share_pct,
    toFloat32(coalesce(round(
        100.0 * b.away_goals / nullIf(toFloat64(b.match_total_goals), 0),
        1
    ), 0.0)) AS opponent_goal_share_pct,
    toFloat32(round(
        coalesce(round(100.0 * b.home_goals / nullIf(toFloat64(b.match_total_goals), 0), 1), 0.0)
        - coalesce(round(100.0 * b.away_goals / nullIf(toFloat64(b.match_total_goals), 0), 1), 0.0),
        1
    )) AS goal_share_delta_pct,
