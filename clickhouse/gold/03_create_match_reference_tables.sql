-- Gold match references for signal/scenario availability.

CREATE TABLE IF NOT EXISTS gold.match_signal_reference
(
    match_id                        Int32,
    match_date                      Date,
    ...
    all_signal_ids                  Array(String),
    available_signal_ids            Array(String),
    unavailable_signal_ids          Array(String),
    signal_count                    UInt16,
    available_signal_count          UInt16,
    has_any_signal                  UInt8,
...
CREATE TABLE IF NOT EXISTS gold.match_scenario_reference
(
    match_id                        Int32,
    match_date                      Date,
    match_time_utc                  Nullable(String),
    ...
    home_score                      Nullable(Int32),
    away_score                      Nullable(Int32),
    all_scenario_ids                Array(String),
    available_scenario_ids          Array(String),
    unavailable_scenario_ids        Array(String),
    scenario_count                  UInt16,
    available_scenario_count        UInt16,
    has_any_scenario                UInt8,
    inserted_at                     DateTime DEFAULT now()
)
ENGINE = ReplacingMergeTree(inserted_at)
PARTITION BY toYYYYMM(match_date)
