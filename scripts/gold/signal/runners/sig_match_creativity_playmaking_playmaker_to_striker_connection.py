"""Run the sig_match_creativity_playmaking_playmaker_to_striker_connection gold query."""
...
TARGET_TABLE = f"{gold_signals_db()}.sig_match_creativity_playmaking_playmaker_to_striker_connection"


def parse_args(argv=None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the match-creativity-playmaking-playmaker-to-striker-connection signal query"
    )
...
def main(argv=None) -> int:
    ...
    target_db = os.getenv("CLICKHOUSE_GOLD_TARGET_DB", gold_signals_db())
    insert_query = insert_query.replace("gold.", f"{target_db}.")
    target_table = TARGET_TABLE.replace(gold_signals_db(), target_db)
    ...
    try:
        ...
        logger.info("sig_match_creativity_playmaking_playmaker_to_striker_connection insert completed")
