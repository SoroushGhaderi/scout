...
class ValidationReportGenerator:
    ...
    def validate_directory(
        self, input_dir: Path, pattern: str = "*.json", recursive: bool = True
    ) -> List[Dict[str, Any]]:
        """
    ...
    def generate_report(
        self, results: List[Dict[str, Any]], report_name: str = "validation_report"
    ) -> Path:
        """
            ...
            summary_records.append(
                {
                    ...
                    "Has Player Stats": result.get("data_completeness", {}).get(
        ...
        for result in results:
            ...
            for error in result.get("errors", []):
                error_records.append(
                    {"File": file_name, "Match ID": match_id, "Type": "Error", "Message": error}
                )

            for warning in result.get("warnings", []):
                error_records.append(
                    {"File": file_name, "Match ID": match_id, "Type": "Warning", "Message": warning}
                )
        ...
        stats = {
            ...
            "With Shotmap": sum(
                1 for r in results if r.get("data_completeness", {}).get("has_shotmap", False)
            ),
            "With Lineup": sum(
                1 for r in results if r.get("data_completeness", {}).get("has_lineup", False)
            ),
            "With Player Stats": sum(
                1 for r in results if r.get("data_completeness", {}).get("has_player_stats", False)
            ),
        }
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Validate FotMob API responses")
    parser.add_argument("input_dir", type=str, help="Directory containing FotMob match files")
    parser.add_argument(
        "--output-dir",
        type=str,
    ...
    parser.add_argument(
        "--pattern", type=str, default="*.json", help="File pattern to match (default: *.json)"
    )
    ...
    results = generator.validate_directory(
        input_path, pattern=args.pattern, recursive=not args.no_recursive
    )
