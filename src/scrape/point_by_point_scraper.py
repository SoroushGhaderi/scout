...
def game_point_by_point_information(
    match_id: int, response_json: json, data_class_schema: dataclass = td.GameInfo
) -> list[dict]:
    """
    ...
    for set_num in range(len(response_json["pointByPoint"])):
        for game_num in range(len(response_json["pointByPoint"][set_num]["games"])):
            for point in range(
                len(response_json["pointByPoint"][set_num]["games"][game_num]["points"])
            ):
                single_game_info = data_class_schema(
                    match_id=match_id,
                    set_id=response_json["pointByPoint"][set_num].get("set"),
                    game_id=response_json["pointByPoint"][set_num]["games"][
                        game_num
                    ].get("game"),
                    point_id=point,
                    home_point=response_json["pointByPoint"][set_num]["games"][
                        game_num
                    ]["points"][point].get("homePoint"),
                    away_point=response_json["pointByPoint"][set_num]["games"][
                        game_num
                    ]["points"][point].get("awayPoint"),
                    point_description=response_json["pointByPoint"][set_num]["games"][
                        game_num
                    ]["points"][point].get("pointDescription"),
                    home_point_type=response_json["pointByPoint"][set_num]["games"][
                        game_num
                    ]["points"][point].get("homePointType"),
                    away_point_type=response_json["pointByPoint"][set_num]["games"][
                        game_num
                    ]["points"][point].get("awayPointType"),
                    home_score=response_json["pointByPoint"][set_num]["games"][
                        game_num
                    ]["score"].get("homeScore"),
                    away_score=response_json["pointByPoint"][set_num]["games"][
                        game_num
                    ]["score"].get("awayScore"),
                    serving=response_json["pointByPoint"][set_num]["games"][game_num][
                        "score"
                    ].get("serving"),
                    scoring=response_json["pointByPoint"][set_num]["games"][game_num][
                        "score"
                    ].get("scoring"),
                )
...
def main(match_id: int, response_json: json):
    point_by_point = game_point_by_point_information(
        match_id=match_id, response_json=response_json
    )
    json_to_parquet(
        json_input=point_by_point,
        write_path=file_path["raw_point_by_point"] + f"pbp_{match_id}" + ".parquet",
    )
