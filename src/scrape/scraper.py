...
def main():
    ...
    if not os.path.isfile(os.path.join(file_path["tracking_scrape"], f"scrape_status_{arguments.date.replace('-', '')}.csv")):
        ...
        scraper_dataframe.to_csv(os.path.join(file_path["tracking_scrape"], f"scrape_status_{arguments.date.replace('-', '')}.csv"), index=False,)
    else:
        scraper_dataframe = pd.read_csv(os.path.join(file_path["tracking_scrape"],f"scrape_status_{arguments.date.replace('-', '')}.csv"))
            ...
            if value[parameter] == "-" and parameter in dict_of_active_scrapes.keys():
                try:
                    ...
                    scraper_dataframe.to_csv(
                        os.path.join(file_path["tracking_scrape"], f"scrape_status_{arguments.date.replace('-', '')}.csv",), index=False)
                except:
