...
class StorageConfig:
    ...
    bronze_path: str = ""  # Must be set explicitly in config classes
    enabled: bool = True
...
class RetryConfig:
    ...
    status_codes: tuple = field(default_factory=lambda: (429, 500, 502, 503, 504))
    ...
    def _ensure_directories(self):
        ...
        if data_path.exists():
            if data_path.is_dir():
                ...
                # Exists as a file - this is a conflict
                raise OSError(
                    f"Cannot create directory 'data': A file with that name already exists. "
                    f"Path: {data_path.absolute()}. Please remove or rename the file."
                )
            else:
                # Exists but is neither file nor directory (symlink, etc.)
                raise OSError(
                    f"Cannot create directory 'data': A non-directory with that name already exists. "
                    f"Path: {data_path.absolute()}. Please remove or rename it."
                )
        else:
            ...
            try:
                ...
                # Race condition: file was created between exists() check and mkdir()
                # Check again what it is now
                if data_path.exists() and data_path.is_dir():
                    # It's a directory now, we're good
                    pass
                else:
                    raise OSError(
                        f"Cannot create directory 'data': A file or non-directory with that name exists. "
                        f"Path: {data_path.absolute()}. Please remove or rename it."
                    ) from e
            except OSError as e:
                if e.errno == 17:  # File exists error (on some systems)
                    if data_path.exists() and data_path.is_dir():
                        # It's a directory now, we're good
                        pass
                    else:
        ...
        # Ensure config-specific directories
        for field_name, field_value in self.__dict__.items():
            if isinstance(field_value, (StorageConfig, LoggingConfig, MetricsConfig)):
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        for field_name, field_value in self.__dict__.items():
            if field_name.startswith('_'):
                continue
            if isinstance(field_value, (StorageConfig, LoggingConfig, MetricsConfig, RetryConfig)):
                result[field_name] = field_value.__dict__
            elif isinstance(field_value, (list, dict, str, int, float, bool, type(None))):
                result[field_name] = field_value
            else:
                # For nested dataclasses
                result[field_name] = field_value.__dict__ if hasattr(field_value, '__dict__') else str(field_value)
        return result
