...
class DataVersioning:
    ...
    def _load_version_metadata(
        ...
        try:
            ...
            self.logger.error(
                f"Could not load version metadata from {metadata_path}: {e}"
            )
            return {}

    def _save_version_metadata(
        self, versions_dir: Path, metadata: Dict[str, List[Dict[str, Any]]]
    ):
        ...
        try:
            ...
            self.logger.error(
                f"Could not save version metadata to {metadata_path}: {e}"
            )
    ...
    def _archive_current_version(
        ...
        try:
            ...
            self.logger.info(
                f"Archived version {version} of {current_file.name} "
                f"to {versioned_file}"
            )
            ...
            self.logger.error(
                f"Could not archive version {version} of "
                f"{current_file.name}: {e}"
            )
            return None
    ...
    def save_version(
        ...
        if match_metadata:
            ...
            if not force_new_version and new_checksum == last_checksum:
                self.logger.debug(
                    f"Data unchanged for {match_id}, skipping versioning"
                )
    ...
    def _compute_checksum_from_file(self, file_path: Path) -> str:
        ...
        try:
            ...
            self.logger.warning(
                f"Could not compute checksum for {file_path}: {e}"
            )
            return ""
    ...
    def get_versions(self, match_id: str, date_str: str) -> List[VersionMetadata]:
        ...
        match_metadata = all_metadata.get(match_id, [])
        return [
            VersionMetadata.from_dict(m)
            for m in sorted(match_metadata, key=lambda x: x['version'])
        ]

    def get_version_file(
        self, match_id: str, date_str: str, version: int
    ) -> Optional[Path]:
    ...
    def restore_version(
        ...
        try:
            ...
            self.logger.error(
                f"Could not restore version {version} for {match_id}: {e}"
            )
            return False
    ...
    def create_scd_record(
        ...
        if not self.enable_scd:
            raise RuntimeError(
                "SCD is not enabled. Set enable_scd=True in DataVersioning constructor."
            )
        ...
        self.logger.info(
            f"Created SCD record version {next_version} for {record_id}"
        )
        return new_record
