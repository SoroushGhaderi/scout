...
class DailyAggregator:
    """Concatenates per-game Silver layer data to daily files.
    ...
    def aggregate_date(self, date_str: str, force: bool = False) -> Dict[str, int]:
        ...
        if not date_dir.exists():
            self.logger.error(
                f"ERROR: Cannot aggregate: Silver layer matches not found for {date_partition}"
            )
        ...
        if not dataframe_dirs:
            self.logger.error(
                f"ERROR: Cannot aggregate: No dataframes found in {date_dir}"
            )
            self.logger.info(" The Silver layer appears empty. Process it first.")
            return {"status": "error", "message": "No dataframes found"}

        self.logger.info(
            f"SUCCESS: Verified Silver layer: {len(dataframe_dirs)} dataframe types found"
        )

        agg_date_dir = self.aggregated_dir / date_partition
        if agg_date_dir.exists() and not force:
            self.logger.info(
                f"Daily aggregates already exist for {date_partition} "
                "(use --force to re-aggregate)"
            )
        ...
        self.logger.info(
            f"Completed concatenation for {date_partition}: "
            f"{total_records} records in {len(stats)} dataframes"
        )

        return stats

    def _concatenate_dataframe(
        self, source_dir: Path, output_dir: Path, df_name: str
    ) -> int:
        ...
        try:
            ...
            self.logger.debug(
                f"Concatenated {len(combined_df)} {df_name} records from {len(files)} files"
            )
