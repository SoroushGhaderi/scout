...
from tables_dataclasses import PeriodInfo
import json


def statistic_information(match_id: int, response_json: json, data_class_schema: PeriodInfo) -> list[dict]:
    """
    ...
    for period in range(len(response_json["statistics"])):
        for group in range(len(response_json["statistics"][period]["groups"])):
            for statistic_category in range(len(response_json["statistics"][period]["groups"][group]["statisticsItems"])):

                single_game_stats = data_class_schema(
                    match_id = match_id,
                    period = response_json["statistics"][period].get("period"),
                    statistic_category_name = response_json["statistics"][period]["groups"][group].get("groupName").lower(),
                    statistic_name = response_json["statistics"][period]["groups"][group]["statisticsItems"][statistic_category].get("name").lower().replace(" ", "_"),
                    home_stat = response_json["statistics"][period]["groups"][group]["statisticsItems"][statistic_category].get("home"),
                    away_stat = response_json["statistics"][period]["groups"][group]["statisticsItems"][statistic_category].get("away"),
                    compare_code = response_json["statistics"][period]["groups"][group]["statisticsItems"][statistic_category].get("compareCode"),
                    statistic_type = response_json["statistics"][period]["groups"][group]["statisticsItems"][statistic_category].get("statisticsType"),
                    value_type = response_json["statistics"][period]["groups"][group]["statisticsItems"][statistic_category].get("valueType"),
                    home_value = response_json["statistics"][period]["groups"][group]["statisticsItems"][statistic_category].get("homeValue"),
                    away_value = response_json["statistics"][period]["groups"][group]["statisticsItems"][statistic_category].get("awayValue"),
                    home_total = response_json["statistics"][period]["groups"][group]["statisticsItems"][statistic_category].get("homeTotal"),
                    away_total = response_json["statistics"][period]["groups"][group]["statisticsItems"][statistic_category].get("awayTotal")
                )
