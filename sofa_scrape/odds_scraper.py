...
import json
from tables_dataclasses import OddsInfo
import pandas as pd


def game_odds_information(match_id: int, response_json: json, data_class_schema: OddsInfo) -> list[dict]:
    """
    ...
    for market in range(len(response_json["markets"])):
        for choice in range(len(response_json["markets"][market]["choices"])):
            single_odd_info = data_class_schema(
                ...
                market_name = response_json["markets"][market].get("marketName").lower().replace(" ", "_"),
                is_live = response_json["markets"][market].get("isLive"),
                suspended = response_json["markets"][market].get("suspended"),
                initial_fractional_value = response_json["markets"][market]["choices"][choice].get("initialFractionalValue"),
                fractional_value = response_json["markets"][market]["choices"][choice].get("fractionalValue"),
                choice_name = response_json["markets"][market]["choices"][choice].get("name"),
                choice_source_id = response_json["markets"][market]["choices"][choice].get("sourceId"),
                winnig = response_json["markets"][market]["choices"][choice].get("winning"),
                change = response_json["markets"][market]["choices"][choice].get("change"),
            )
